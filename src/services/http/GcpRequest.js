...
export const uploadImage = async (carpeta, fileName, id, imageFile) => {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Solicitud de carga de imagen fallida con código de estado: ${response.status}`
            )
        }
...
export const deleteImage = async (carpeta, id) => {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Solicitud de eliminación de imagen fallida con código de estado: ${response.status}`
            )
        }
...
export const getImage = async (carpeta, id) => {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Solicitud de obtención de imagen fallida con código de estado: ${response.status}`
            )
        }
