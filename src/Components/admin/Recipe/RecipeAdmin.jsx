...
import PublicationForm from "./RecipeForm";
...
export default function BlogAdmin() {
  ...
  const handleEdit = (id) => {
    const publicationToEdit = blogData.find((item) => item.idReceta === id);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const updatedBlogData = blogData.filter((item) => item.idReceta !== id);
  ...
  return (
    <div className="container mx-auto mt-8 bg-gray-900 rounded-lg text-gray-500 dark:text-gray-400">
      ...
      <div className="grid grid-cols-7 gap-4 bg-gray-900">
        ...
        <div className="col-span-1 font-bold text-center">ingredientes</div>
        <div className="col-span-1 font-bold text-center">elaboracion</div>
        <div className="col-span-1 font-bold ">Foto URL</div>
        <div className="col-span-1 font-bold text-center">Acciones</div>

        {/* Contenido de la tabla */}
        {blogData.map((item) => (
          <React.Fragment key={item.idReceta}>
            <div className="col-span-1 text-center">{item.idReceta}</div>
            ...
            <div className="col-span-1 text-center">
              {truncateText(item.ingredientes)}
            </div>
            <div className="col-span-1 text-center">
              {truncateText(item.elaboracion)}
            </div>
            ...
            <div className="col-span-1 text-center">
              ...
              <button
                onClick={() => handleActionClick("edit", item.idReceta)}
