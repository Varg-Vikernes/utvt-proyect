...
function Recetas() {
    ...
    /* Función que envía una solicitud de eliminación a la API cuando el usuario hace clic en el botón de eliminar. Puedes usar el método HTTP DELETE unción que envíe una solicitud de eliminación a la API cuando el usuario haga clic en el botón de eliminar. Puedes usar el método HTTP DELETE  */
    const handleDeleteClick = async (recetaId) => {
        try {
            const token = localStorage.getItem('tokenSession')

            const response = await fetch(
                `https://backend-proyecto-api-production.up.railway.app/recetas/${recetaId}`,
                {
                    method: 'DELETE',
                    headers: {
                        Authorization: `Bearer ${token}`, // Incluye el token en el encabezado de autorización
                        'Content-Type': 'application/json', // Ajusta según el tipo de contenido que tu backend acepte
                    },
                }
            )

            if (!response.ok) {
                throw new Error(`Error al eliminar receta: ${response.status}`)
            }

            // Actualiza el estado solo si la eliminación fue exitosa
            setRecetas((prevRecetas) =>
                prevRecetas.filter((receta) => receta.idReceta !== recetaId)
            )
        } catch (error) {
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg ">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-auto">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-2xl w-24">
                            Nombre Receta
                        </th>
                        <th scope="col" class="px-6 py-3 text-2xl max-w-xs">
                            Descripción
                        </th>
                        <th scope="col" class="px-6 py-3 text-2xl max-w-xs">
                            Ingredientes
                        ...
                        <th scope="col" class="px-6 py-3 text-2xl justify-evenly">
                            Acciones
                            <div>
                                <button
                ...
                <tbody>
                    {recetas.map((receta) => (
                        <tr
                            key={receta.idReceta}
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-xl">
                            <th
                                scope="row"
                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {receta.titulo}
                            </th>
                            <td class="px-6 py-4 text-xl text-justify">{receta.descripcion}</td>
                            <td class="px-6 py-4 text-xl text-justify">{receta.ingredientes}</td>
                            <td class="px-6 py-4 text-xl text-justify">{receta.elaboracion}</td>
                            <td class="p-4 whitespace-nowrap space-x-2">
                                ...
                                <button
                                    onClick={() => handleDeleteClick(receta.idReceta)}
                                    type="button"
                                    data-modal-toggle="delete-user-modal"
                                    className="text-white bg-red-600 hover:bg-red-800 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
                                    <svg
